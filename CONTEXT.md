...
| 13 | Your Own Live Dashboard | live |

**Activity 13 is not in the original Classroom list.** It was added on 14 Sep 2026 from Kamil's
own draft page and from the dashboard and Apps Script files that were already sitting in the
repository root. If the Classroom is ever renumbered to include it, the Classroom still wins —
but the site's URL `activity-13.html` is now published, so do not move it.

**Every number from 0 to 13 now exists**, and the run is contiguous. Activity 0 and Activity 3
...
docs/activity-N.html   one page per activity (students), N = 0 to 13
...
docs/style.css         the whole design system — every page links it. GENERATED:
                       it is style-partA.css + style-partB.css. See §4.1.
docs/style-partA.css   page furniture (tokens, top bar, rail, hero, cards, footer)
docs/style-partB.css   diagram and widget families — untouched by the 2026 redesign
...
docs/robots.txt        keeps teacher pages out of search engines
                       (lists teacher.html and teacher-0 … teacher-13)
...
docs/img/stemlab-*.svg / .png     the UMPSA STEM LAB logo set, vector (see §4.6)
docs/img/favicon.svg              the emblem, as the site favicon
```
...
**Rebuilt on 14 Sep 2026.** The colours, the class names and the See · Think · Explore · Marvel
strip all survived; the page furniture around them did not. If you are reading old notes that
describe a plain two-column page with a teal bar, that page is gone.

### 4.1 Where the CSS lives

`docs/style.css` is generated by concatenating two files that are also in the repo:

```
docs/style-partA.css   tokens, brand ribbon, top bar, module rail, hero, cards,
                       callouts, steps, quiz, landing grid, footer
docs/style-partB.css   the diagram and widget families, unchanged since Activity 12
docs/style.css         partA + partB, and the file every page links
```

**Edit the part, then rebuild the whole.** Part B is the `.dia` classes, the Wokwi-look board
families, the per-activity widgets and the board explorer — hundreds of rules that nothing in the
redesign touched. Keeping them in their own file is what made the redesign safe to do, and
splitting them again later would be a lot of work. Concatenating is one line:

```python
open('docs/style.css','w').write(open('docs/style-partA.css').read() + '\n'
                                 + open('docs/style-partB.css').read())
```

### 4.2 Colour

Brand colours, sampled from the UMPSA STEM LAB emblem and now identical to it: teal `#02ADA5`,
blue `#0D55A7`, red `#C80000`, orange `#F5A21D`, plus the emblem's leaf green `#64A026` and amber
`#FBA919`. `--green:#1F9D55` is kept as the *success* colour and is deliberately not the leaf
green — it is what a ticked checkbox and a right quiz answer use.

**Never hard-code a text or background colour.** Use `var(--ink)`, `var(--muted)`, `var(--card)`,
`var(--bg)`, `var(--line)`, `var(--sink)`. All of them flip for dark mode.

**Every page carries an accent**, set as `data-accent` on `<body>` and used by the hero gradient,
the step numbers, the rail highlight, the progress bar and every hover state. It follows the phase
of the module rather than the activity:

| Phase | Activities | `data-accent` |
|---|---|---|
| Start here | 0, 1, 2 | `teal` |
| Make things happen | 3, 4, 5, 6 | `orange` |
| Sense the world | 7, 8, 9 | `leaf` |
| Get online | 10, 11 | `blue` |
| Go further | 12, 13 | `red` |

`build_shell.py` writes the attribute, so do not set it by hand. `data-accent` also works on any
element — `index.html` puts it on each phase's card grid.

### 4.3 The page shell, and the script that writes it

Every page's shell is **generated**, not hand-maintained:

```
build_shell.py     head links, brand ribbon, top bar, the module rail, the footer
```

It is **idempotent** — run it as often as you like. It rewrites the block between
`<div class="chev">` and the end of the top bar, re-wraps the main content, refreshes the rail and
replaces the footer, and leaves everything in between untouched. **Adding an activity is now one
edit**: add a row to `ACTS` at the top of `build_shell.py` and run it. That replaces the old rule
that a new activity meant hand-editing a nav block and a footer on twenty-nine pages.

Page order, on every page:

```
<a class="skip">           skip link
<div class="chev">         the four brand words — still the staff door, see §9.1
<header class="sitehead">  emblem, LilEx5 wordmark, Home / activity dropdown / Pin reference
<header class="hero">      full-bleed, accent gradient, circuit pattern
<div class="progress-bar"> activity pages only
<div class="shell">
  <aside class="rail">     the module map — 14 activities, grouped by phase
  <main class="wrap" id="main">   the page
<footer>                   the full STEM LAB lockup and the link list
```

**The rail is the visible change.** It is sticky, shows every activity grouped by phase, marks the
current page, and `code.js` ticks the ones the student has already started by reading
`localStorage['lilex5v2:activity-N.html']`. It appears at **1180 px and wider** and is simply
`display:none` below that, where the dropdown in the top bar does the same job. Nothing depends on
it existing.
...
that `build_shell.py` puts in every head. Body text stays on the system stack. If the font fails
to load the page falls back and nothing breaks — the container this site was built in cannot reach
Google Fonts, and the build was verified that way.

### 4.6 The logo

`docs/img/` holds a full logo set, rebuilt as vector on 14 Sep 2026 from two low-resolution
screenshots Kamil supplied:

```
stemlab-mark.svg                the emblem alone — used in the top bar
stemlab-logo.svg                horizontal lockup, dark text (light backgrounds)
stemlab-logo-light.svg          horizontal lockup, white text — used in the footer
stemlab-logo-stacked.svg        vertical lockup, dark text
stemlab-logo-stacked-light.svg  vertical lockup, white text
favicon.svg                     the emblem with a heavier white edge, for small sizes
stemlab-*.png                   transparent PNG exports, 1024–2400 px, for decks and print
```

How they were made, in case they ever need remaking: each flat colour region was masked out of the
source PNG and traced to bezier paths; the ten green leaves and the four coloured petals were then
**replaced with ideal two-arc leaf shapes** measured from those masks (centre, axis, and each
side's depth separately, so the asymmetry survives), because a direct trace of a 512-pixel image
leaves lumpy edges and spurs at the tips. The three gears are drawn from scratch. The circuit-board
icon is the original routing, skeletonised and redrawn as uniform-width strokes with ten perfect
ring pads. The wordmark is **Liberation Sans Bold converted to outlines** — metrically identical to
the Arial Bold the original used, and with no font dependency at all. Scripts are not in the repo;
they were throwaway. **Do not go back to the raster versions**: they have a photographic
circuit-board background baked in that cannot be cleanly removed.
...
**Activity 13 is the one deliberate exception, and it was Kamil's call.** That activity ships a
finished file pack (`wifi_reporter.py`, `main_dashboard.py`, the Apps Script, two dashboard files)
because the new learning there is entirely operational — deploying a service, keeping an address
safe, choosing a sampling rate — and the Python is three files of sensor plumbing the student has
already met. **The rule itself still holds on the page**: every line of code shown on
`activity-13.html` is a `.codeimg` picture, the four `config.py` lines go through the typer, and
the copy test passes. The exception is about what is *handed over*, not about what is *displayed*.
...
### Activity 13 — Your Own Live Dashboard

Built on 14 Sep 2026 from a draft page Kamil supplied, plus `LilEx5_Dashboard.html`,
...
- **The one idea is that the middle is yours.** It completes a three-lesson arc that should be said
  ...
  carry it.
- **The honest cost is taught beside the win**, as in Activity 12: you need an account, the address
  is a password, Apps Script is slower than MQTT, and a deleted deployment fails silently. A student
  is asked to name one thing that got *harder*.
- **Second idea: history.** A gauge answers *what is it now*; a sheet answers *what has it been
  doing*. Nothing earlier in the module could do the second, and that is what makes a store worth
  more than a display.
- **POST is contrasted with publish**, deliberately, as the technical half of the lesson. Activity 10
  handed a message over and never found out what happened; today the board waits for an answer, so
  failures are loud instead of silent. Both shapes are real and the class has now written one of each.
- **JSON arrives as "a reading with its name on it".** The name is the *only* agreement between the
  board and the sheet; a misspelled name silently creates a new column. That is drawn, it is in the
  troubleshooting table, it is a quiz question, and `teacher-13.html` asks the teacher to break it
...
- **The dashboard's address is set by editing three constants at the top of the HTML file**
  ...
  was written from the file, not the draft. If the dashboard is ever rewritten to have a settings UI,
  Part 4 of the student page has to change with it.
- **The draft also named `LilEx5_Dashboard4.html`**, which does not exist. The real names are
  `LilEx5_Dashboard.html` and `LilEx5_Dashboard_History.html`, and the page uses those.
- Mission: **every sensor onto one live page, through a script you deployed, into a sheet you own.**
  Exercise: **one reading a minute, and a dashboard that opens on a whole school day** — two numbers,
  in two files, and the second is *derived* from the first (60000 ms, and 7 × 60 = 420 rows). It
  needs no new syntax, it is checkable in two minutes from the Shell, and it teaches sampling rate,
  which the module had never touched. **No answer on the student page** (rule 5.4); three clues, the
  last a five-point checklist. The answer and the "what if you go back to five seconds" follow-up
  question are on `teacher-13.html`.
- **Ten diagrams**, all rendered and looked at in both themes: the round trip, whose computer is in
  the middle, publish versus post, a name becoming a column, the three deployment settings, the
  anatomy of the `/exec` address, the Shell, the sheet filling, live versus history, and the
  exercise's expected output. Two needed fixing after seeing them (a dashed connector that stopped
  short of its arrow, and a caption sitting on a panel border).
- **`activity.js` and `style.css` were not touched for this activity.** No new widget: the typer,
  quiz and progress bar are the stock ones, and the typer's fourth line is the JSON shape rather than
  a config line, so students meet the curly brackets and the colon before they see them in the sheet.
- **No `for` loops and no `elif`** appear on the page, still.

#### The security finding, which is real and is not fixed

`LilEx5_Dashboard.html` and `LilEx5_Dashboard_History.html` in the repo root **ship with a live
`/exec` address typed into `API_URL`**. The repository is public, so anybody who reads it can add
rows to whichever sheet that deployment writes to — Apps Script web apps set to *Anyone* have no
other protection. `teacher-13.html` carries a red callout telling the teacher to replace it with a
placeholder before the pack goes out, or to deploy a throwaway script for the class. **The student
page never prints an address**, and the module's own advice is the same advice, which is the reason
this is worth acting on rather than leaving.
...
### 8.2 Adding an activity is now one edit, not thirty

**This section used to say the opposite and it is out of date.** Since 14 Sep 2026 the nav, the
module rail and the footer on all thirty-one pages are written by `build_shell.py`. Adding
activity N means:

1. add a row to `ACTS` at the top of `build_shell.py` (number, short name, accent);
2. run `python3 build_shell.py`;
3. add the `teacher-N.html` line to `robots.txt` by hand — the one list the script does not write;
4. fix the **previous** activity's `.stepnav` "next" card, which still points at
   `index.html#activities`, and any "this is the last activity" wording in its quiz-win box;
5. add the row to `index.html`'s `.actlist` and to `teacher.html`'s table.

The short names live in three places — the dropdown, the rail (both generated) and `index.html`'s
`.actlist` (hand-written). Change one and change all three.
...
`robots.txt` lists `teacher.html` and `teacher-0.html` through `teacher-13.html`. Activity 11's
build extended it from 9, Activity 12's added `teacher-0.html`, and Activity 13's added
`teacher-13.html`. A new teacher page **does** need a line here — it is the one list
`build_shell.py` does not write.
...
4. Add the activity to `ACTS` in `build_shell.py` and run it — that does the nav, the rail and
   the footer on every page (§8.2). Then `robots.txt`, and the previous activity's stepnav.
5. `docs/index.html`: add the `.act` card to the right phase group in `.actlist`, with
   `data-act="N"` so the progress tick works.
